<script lang="ts">
    ...
    function filterPlayers(players: string): RankedUser[] {
        if (players == '') {
            ...
            return orderedPlayers.map((player) => {
                player.show = queryPlayersArray.reduce(
                    (acc, current) => acc || (player.name.toUpperCase().includes(current.toUpperCase()) && current !== ''),
                    false
                );
                return player;
            });
    ...
    function sortPlayers(label: string) {
        ...
        orderedPlayers = orderedPlayers.sort((a, b) => {
            if (currentSortLabel === 'rank') {
                ...
                return currentSortOrder === 'asc'
                    ? a.user_score.results_good - b.user_score.results_good
                    : b.user_score.results_good - a.user_score.results_good;
            } else {
...
<div class="w-full h-full flex flex-row m12:flex m12:flex-col m12:items-center">
    ...
    <div class="flex flex-col h-full overflow-hidden mt-4 m8:mt-0 w-5/6 m12:w-full">
        <RankingBanner sortingFunction={sortPlayers} {currentSortLabel} {currentSortOrder} />
