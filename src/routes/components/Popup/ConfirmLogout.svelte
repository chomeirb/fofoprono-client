...
<div id="window" class="fixed z-50 flex h-[100vh] w-[100vw] flex-row items-center justify-center bg-primary bg-opacity-50 text-3xl text-secondary m12:text-xl">
	<div
		...
		<div class="flex flex-row justify-between">
			<button class="m-2 w-60 rounded-3xl bg-primary p-2 text-secondary hover:opacity-60 dark:bg-secondary dark:text-primary" on:click={logout}>Oui</button>
			<button class="m-2 w-60 rounded-3xl bg-primary p-2 text-secondary hover:opacity-60 dark:bg-secondary dark:text-primary" on:click={hideFunction}>Annuler</button>
		</div>
	</div>
</div>
